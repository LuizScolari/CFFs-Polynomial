...
def generate_polynomials(GF1, GF2, k, growth):
    if growth == "first":
        ...
        [polynomials_new.append(galois.Poly(list(vector), field=GF2)) for vector in polynomial_vectors]
        return polynomials_old_new, polynomials_new
...
def generate_combinations(GF1, GF2, growth):
    if growth == "first":
        ...
        return combinations_old_new, combinations_new
...
def evaluate_polynomials(GF1, GF2, k, growth):
    if growth == "first":
        ...
        polynomials_old_new, polynomials_new = generate_polynomials(GF1, GF2, k, growth)
        combinations_old_new, combinations_new = generate_combinations(GF1, GF2, growth)
